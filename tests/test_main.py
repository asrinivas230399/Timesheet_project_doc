...
async def test_get_task_updates():
    ...
    assert response.status_code == 200
    assert response.json() == {"task": "Update routing", "status": "Completed"}

@pytest.mark.asyncio
async def test_update_task():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.put("/task/1", params={"status": "In Progress"})
    assert response.status_code == 200
    assert response.json() == {"task_id": 1, "status": "In Progress"}
